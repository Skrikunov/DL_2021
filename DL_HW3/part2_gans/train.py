...
class GAN(pl.LightningModule):
    def __init__(
        ...
        if self.truncation_trick:
            ...
            size = (len(self.train_dataset), self.noise_channels)
            trunc_normal = stats.truncnorm.rvs(-1, 1,size=size)
            self.val_noise.data = torch.tensor(trunc_normal).float()
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        noise = torch.randn(labels.shape[0], self.noise_channels).to(device)
        fake = self.dis(self.gen(noise, labels), labels).to(device)
        real = self.dis(imgs, labels).to(device)

        if optimizer_idx == 0:
            loss = self.loss(fake)
        elif optimizer_idx == 1:
            loss = self.loss(fake_scores=fake,real_scores=real)
